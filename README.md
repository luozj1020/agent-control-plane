...
argv validation commands, Task Revisions, and evidence hashes must be machine
validated. The upstream Main Agent is the normal Task Card author. ACP owns
persistence, validation, versioning, freeze, inspection, and later execution
binding; it does not translate the human's natural-language request into a
Task Card.
...
The local Workspace Task Store provides the durable upstream handoff:

```bash
agent-control-plane task create \
  --workspace /absolute/repository/path \
  --task-id ANNC-123 \
  --task TASK.json \
  --source codex

agent-control-plane task validate \
  --workspace /absolute/repository/path \
  --task-id ANNC-123 \
  --expected-working-copy-generation 1

agent-control-plane task freeze \
  --workspace /absolute/repository/path \
  --task-id ANNC-123 \
  --expected-working-copy-generation 1
...
└── tasks/<taskId>/
    ├── working-copy.json
    ├── metadata.json
    ├── revisions/task-revision-0001.json
    ├── preflights/<preflightId>.json
    └── deltas/<revisionDeltaId>.json
```

The mutable working copy has `workingCopyGeneration` and
`baseTaskRevision`, but never `taskRevision`. Writes require
`expectedWorkingCopyGeneration` and fail on a CAS mismatch. Validation binds
the current generation to the canonical Task Card hash. Only a successful
freeze allocates `taskRevision`, writes a non-overwriting immutable artifact,
updates relationship/lifecycle metadata, and then publishes `active-task.json`.
The active pointer contains only `workspaceId`, `taskId`, `taskRevision`, and
`taskSha256`; runtimes therefore cannot consume a draft by accident. The four
names `workspaceRevision`, `taskRevision`, `workingCopyGeneration`, and
...
The CLI refuses to overwrite an existing scaffold, Task Revision, or
migration/render target. The Task Card page is a Workspace lifecycle Inspector.
It lists locally persisted tasks, prefers the active frozen contract, otherwise
opens the latest working copy, and exposes `workingCopyGeneration`,
`baseTaskRevision`, `taskRevision`, contract hash, and source without exposing
the local state directory. A frozen Task Revision is always read-only. Draft
...
unpersisted fallback for upstream Agents, CLI use, and manual recovery. Scaffold
editing/export never allocates a Task Revision or writes `active-task.json`.
The structured editor uses collapsible sections for identity,
...
The page also has a Preflight console. It verifies the canonical Task Card,
...
task SHA-256 and runtime envelope. A draft or Scaffold receives diagnostic-only
results. An active frozen Task produces an immutable Preflight Receipt bound to
`workspaceRevision`, configuration SHA-256, activation id, Effective Skill
SHA-256, Task Revision, and runtime-envelope SHA-256. Preflight never launches
an Agent; the actual submit/run action remains a separate, explicit operation.
New Balanced and Overnight runs reject raw Task JSON and missing, stale, or
...
├── active-task.json   # frozen Task reference, when one exists
├── tasks/             # mutable drafts and immutable Task Revisions
...
```bash
agent-control-plane task preflight \
  --workspace /absolute/repository/path \
  --task-id ANNC-123 \
  --workflow-mode overnight \
  --adapter claude-code \
  --strategy convergent \
...
agent-control-plane overnight review --run RUN_DIR --decision revise \
  --workspace /absolute/repository/path --delta REVISION-DELTA.json
```
...
The activated Skill freezes a Workspace Task, creates a Preflight Receipt from
the active configuration, then invokes the exact receipt:
...
envelope rather than entering this task contract. The envelope is frozen by
the Preflight Receipt and cannot be overridden at run creation:
...
agent-control-plane balanced review --run RUN_DIR --decision revise \
  --workspace /absolute/repository/path --delta REVISION-DELTA.json
```

Revision consumes a main-review call and a new downstream round while
preserving the protected final-review slot. `REVISION-DELTA.json` contains
`summary`, non-empty `changes`, optional affected-path/acceptance/evidence
lists, and the complete bounded candidate `task`. ACP validates the candidate,
atomically freezes the next Task Revision, writes an immutable Delta whose
`resultTask` identity is generated only after freeze, creates a new Preflight
Receipt, and binds all three artifacts to the next round. It reuses the prior downstream
...
```json
{
  "summary": "Tighten the remaining implementation boundary.",
  "changes": ["Repair only the rejected behavior."],
  "affectedPaths": ["src/feature/**"],
  "affectedAcceptanceIds": ["acceptance-2"],
  "requiredEvidence": ["Re-run validation-2."],
  "task": { "id": "task-id", "...": "complete canonical Task Card" }
}
```

The input does not declare `revisionDeltaId`, `taskRevision`, or `resultTask`;
the Workspace Task Store allocates and hash-binds those identities. The initial
run binding remains immutable, while each later round/cycle appends a distinct
revision binding and snapshots its Delta and Preflight Receipt beside the
round/cycle contract.
...
AGENT_CONTROL_CLAUDE_COMMAND=/absolute/path/to/claude agent-control-plane task preflight ...
```
