...
class TestCoreExtensionContracts(unittest.TestCase):
    ...
    def test_interfaces_require_declared_participants_owner_and_validation(self):
        task = _make_valid_task(extensions={
            "task_shape": {
                "participants": [
                    {
                        "id": "producer",
                        "owner": "worker",
                        "responsibilities": ["produce normalized data"],
                    },
                    {
                        "id": "consumer",
                        "owner": "tester",
                        "responsibilities": ["consume normalized data"],
                    },
                ],
                "interfaces": [{
                    "id": "normalized-data",
                    ...
                    "consumer": "consumer",
                    "owner": "consumer",
                    "contract": "normalized fields remain stable",
                    "validation_id": "val-1",
                }],
            },
        })
        self.assertEqual(self.ts.validate_task(task), [])
        self.assertEqual(self.ts.assess_task_granularity(task)["responsibility_count"], 2)
        rendered = self.ts.render_task_card(task, view="execution")
        self.assertIn("Coordination Interfaces", rendered)
        self.assertIn("normalized-data", rendered)

        task["extensions"]["task_shape"]["interfaces"][0]["owner"] = "missing"
        errors = self.ts.validate_task(task)
        self.assertTrue(any("owner" in error and "unknown participant" in error for error in errors))

    def test_interface_endpoints_must_differ_and_validation_must_exist(self):
        task = _make_valid_task(extensions={
            "task_shape": {
                "participants": [
                    {"id": "one", "owner": "worker", "responsibilities": ["one"]},
                    {"id": "two", "owner": "worker", "responsibilities": ["two"]},
                ],
                "interfaces": [{
                    "id": "seam",
                    "producer": "one",
                    "consumer": "one",
                    "owner": "one",
                    "contract": "one-way contract",
                    "validation_id": "missing",
                }],
            },
        })
        errors = self.ts.validate_task(task)
        self.assertTrue(any("producer and consumer must differ" in error for error in errors))
        self.assertTrue(any("unknown validation id 'missing'" in error for error in errors))
