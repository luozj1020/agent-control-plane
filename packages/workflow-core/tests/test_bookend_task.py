...
_SPEC = importlib.util.spec_from_file_location("bookend_task", SCRIPT)
bookend = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(bookend)
...
def write_executor(path, mode):
    path.write_text(
        ...
        if mode == "fail-then-succeed":
            round_epoch = int(os.environ.get("AIWF_BOOKEND_ROUND_EPOCH", "1"))
            if round_epoch <= 1:
                print(json.dumps({{
                    "status": "failed",
                    "bookend_state": "runtime_blocked",
        ...
        if mode == "persist-state":
            ...
            if prior and epoch > 1:
                ...
                (run_dir / "dispatch.stdout").write_text("Diff: " + str(diff) + "\\n", encoding="utf-8")
        ...
        if mode == "slow":
            time.sleep(1.0)
        if mode == "slow-epoch":
            # Check if a review window is set.  If so, simulate a window
            # expiry by returning checkpoint_trigger with product_worktree.
            # In real usage the dispatcher manages the window; here we skip
            # the actual sleep to keep CI tests fast.
        ...
        diff.write_text("diff --git a/a.py b/a.py\\n@@ -1 +1 @@\\n-old\\n+new\\n", encoding="utf-8")
        nl = "\\n"
        (run_dir / "dispatch.stdout").write_text("Diff: " + str(diff) + nl, encoding="utf-8")
        for name in (
...
class BookendTaskTests(unittest.TestCase):
    ...
    def test_done_candidate_emits_complete_projection_and_wake_request(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Codex review is not scheduled", review.stderr)
    ...
    def test_review_input_revokes_wakeup_when_bound_diff_drifts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pending["state"] = "revision_pending"
            pending["terminal"] = False
            pending["codex_wakeup_required"] = True
    ...
    def test_runtime_failure_continues_across_epochs_until_budget(self):
        with tempfile.TemporaryDirectory() as tmp:
            proc, state = self.run_submit(tmp, mode="runtime", max_epochs=2)
            self.assertEqual(proc.returncode, 1, proc.stderr)
            self.assertEqual(state["state"], "runtime_blocked")
            self.assertEqual(state["epoch"], 2)
            state_path = Path(state["control_dir"]) / "bookend-state.json"
    ...
    def test_balanced_mode_cli_help(self):
        """aiwf balanced --help should show the balanced profile option."""
        proc = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "aiwf.py"), "balanced", "--help"],
    ...
    def test_balanced_mode_preview(self):
        """aiwf balanced --preview should run without model calls."""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            repo = tmp_path / "repo"
            init_git_repo(repo)
            ...
            proc = subprocess.run(
                [sys.executable, str(ROOT / "scripts" / "aiwf.py"), "balanced",
                 str(task), "--repo", str(repo), "--profiles-dir", str(PROFILES),
            ...
            self.assertEqual(result.get("status"), "routed")
    ...
    def test_revision_loop_invariant(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cmd = [
                ...
                "--repo", str(repo), "--profiles-dir", str(PROFILES),
                "--run-dir-base", str(tmp_path / "runs"),
    ...
    def test_review_verdict_is_bound_to_current_revision_round(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state_path = root / "bookend-state.json"
            state_path.write_text(json.dumps({
                "schema_version": 1,
                "logical_task_id": "round-bound",
                "contract_hash": "sha256:" + "1" * 64,
                "revision_count": 1,
                "epoch": 2,
            }), encoding="utf-8")
            verdict_path = root / "review-verdict.json"
            stale = {
                "schema_version": 1,
                "kind": "review-verdict",
                "logical_task_id": "round-bound",
            ...
            current = dict(stale, revision_count=1, action="accept")
            writer = threading.Timer(
                0.05,
                lambda: verdict_path.write_text(
                    json.dumps(current), encoding="utf-8"
                ),
            )
            writer.start()
            try:
                verdict = bookend.wait_for_review_verdict(
                    state_path, poll_interval=0.01
                )
            finally:
                writer.cancel()
            ...
            receipt_path = bookend.build_review_receipt(state_path, verdict, {})
            receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
            snapshot = root / "review-verdict-001.json"
            self.assertEqual(receipt["review_verdict"], str(snapshot))
            self.assertTrue(snapshot.is_file())
            self.assertEqual(
                receipt["review_verdict_sha256"], bookend.sha256_file(snapshot)
            )
