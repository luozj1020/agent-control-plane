...
class TestRunWorkflowBalancedMode(unittest.TestCase):
    """Balanced is a repeatable tuned Claude round with a Codex handoff."""

    def test_dispatcher_exports_effective_tuned_time_plan(self):
        dispatcher = (SCRIPTS / "dispatch-to-claude.sh").read_text(
            encoding="utf-8"
        )
        for field in (
            ...
            self.assertIn(f'"{field}"', dispatcher)

    def test_balanced_runs_evidence_and_review_handoff_phases(self):
        with tempfile.TemporaryDirectory() as tmp:
            task_path = write_task(tmp, make_task(task_id="balanced-sequence"))

            def record(name):
                def phase(ctx):
                    ctx.phase_order.append(name)
                return phase

            names = (
                "lint", "compose", "validate", "facts", "route", "context",
                "plan", "dispatch", "evidence", "acceptance", "balanced-review",
            )
            patches = {
                "phase_" + name.replace("-", "_"): record(name)
                for name in names
            }
            with mock.patch.multiple(run_workflow, **patches):
                result = run_workflow.run_lifecycle(
                    task_path=task_path,
                    run_dir_base=Path(tmp) / "runs",
                    repo=ROOT,
                    profiles_dir=PROFILES,
                    profile="balanced",
                )
            ...
            self.assertEqual(result["workflow_mode"], "balanced")
            self.assertEqual(result["final_decision"], "codex-review-required")
            self.assertEqual(result["phases_completed"], list(names))

    def test_balanced_review_binds_evidence_and_tuned_window_observations(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            init_git_repo(root)
            run_dir = root / "run"
            run_dir.mkdir()
            task_path = write_task(tmp, make_task(task_id="balanced-review"))
            ctx = run_workflow.RunContext(
                task_path=task_path,
                run_dir=run_dir,
                repo=ROOT,
                profile="balanced",
            )
            ctx.task_data = {"id": "balanced-review"}
            ctx.acceptance = {"status": "incomplete"}
            (run_dir / "evidence.json").write_text(
                json.dumps({"evidence_hash": "sha256:test"}), encoding="utf-8"
            )
            (run_dir / "acceptance-result.json").write_text(
                json.dumps(ctx.acceptance), encoding="utf-8"
            )
            (run_dir / "claude-phase-metrics.json").write_text(
                json.dumps({
                    "execution_profile": "balanced",
                    "context_acquisition_timeout_seconds": 600,
                    "active_execution_window_seconds": 600,
                    "hard_timeout_seconds": 1500,
                    "first_progress_timeout_seconds": 600,
                    "no_output_timeout_seconds": 600,
                    "active_progress_extension_seconds": 180,
                    "growing_progress_extension_seconds": 600,
                    "growth_extension_policy":
                        "renewable-product-growth-until-hard-timeout",
                    "context_acquisition_seconds": 37,
                    "implementation_seconds": 91,
                    "tail_seconds": 8,
                    "completion_ready_observed": True,
                }),
                encoding="utf-8",
            )

            run_workflow.phase_balanced_review(ctx)

            review = json.loads(
                (run_dir / "balanced-review.json").read_text(encoding="utf-8")
            )
            self.assertEqual(review["round_status"], "review_pending")
            self.assertTrue(review["codex_review_required"])
            self.assertFalse(review["merge_authorized"])
            self.assertFalse(review["time_window_plan"]["fixed_single_checkpoint"])
            self.assertEqual(
                review["time_window_plan"]["active_execution_window_seconds"],
                600,
            )
            self.assertEqual(review["time_window_plan"]["hard_timeout_seconds"], 1500)
            self.assertEqual(
                review["time_window_plan"]["growth_extension_policy"],
                "renewable-product-growth-until-hard-timeout",
            )
            self.assertEqual(
                review["time_window_plan"]["context_acquisition_seconds_observed"],
                37,
            )
            self.assertIsNotNone(review["evidence"]["sha256"])
            self.assertIsNotNone(review["acceptance"]["sha256"])
            self.assertEqual(review["allowed_decisions"], ["accept", "revise", "stop"])
            self.assertEqual(review["round_kind"], "initial")
            self.assertIn("reviewed-continuation", review["next_round_protocol"][1])

    def test_balanced_revision_passes_one_use_continuation_to_dispatcher(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = root / "run"
            run_dir.mkdir()
            approval = root / "approval.json"
            approval.write_text('{"status":"available"}\n', encoding="utf-8")
            task_path = write_task(root, make_task(task_id="balanced-revision"))
            ctx = run_workflow.RunContext(
                task_path=task_path,
                run_dir=run_dir,
                execute=True,
                repo=ROOT,
                profile="balanced",
                reviewed_continuation=approval,
            )
            ctx.task_data = make_task(task_id="balanced-revision")
            ctx.facts = {"task_id": "balanced-revision", "task_type": "implementation"}
            ctx.routing = {"lane": "standard"}
            ctx.composed = {"mode": "builder"}
            dispatch_card = run_dir / "delegation-task-card.md"
            dispatch_card.write_text("# Task\n", encoding="utf-8")
            ctx.execution_plan = {
                "delegation_task_card": str(dispatch_card),
                "execution": {
                    "single_pass_allowed": False,
                    "claude_role": "execution-builder",
            ...
            with mock.patch.object(
                ...
                run_workflow.phase_dispatch(ctx)

            command = runner.call_args.args[0]
            self.assertIn("--reviewed-continuation", command)
            self.assertEqual(
                command[command.index("--reviewed-continuation") + 1],
...
class TestRunWorkflowRegistration(unittest.TestCase):
    ...
    def test_submit_is_primary_and_run_is_compatibility(self):
        ...
        self.assertIn("full foreground compatibility lifecycle", aiwf_content)
