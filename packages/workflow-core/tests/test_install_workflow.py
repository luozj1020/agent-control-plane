...
class InstallWorkflowTests(unittest.TestCase):
    ...
    def test_install_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((repo / "ai" / "audit-codex-wakeups.py").exists())
            self.assertTrue((repo / "ai" / "bookend-task.py").exists())
            ...
            self.assertTrue((repo / "ai" / "schemas" / "solution-contract-v2.schema.json").exists())
    ...
    def test_installed_agent_rules_include_execution_contracts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Select exactly one execution mode", agents)
            self.assertIn("python ai/aiwf.py submit TASK.json", agents)
            self.assertIn("Codex episode ends", agents)
            self.assertIn("## Balanced Foreground Rounds", agents)
            self.assertIn("rounds is task-dependent", agents)
            self.assertIn("## Interactive Codex Ownership", agents)
            self.assertIn("native subagents", agents)
            self.assertIn("review_ready", agents)
            self.assertIn("semantic_blocked", agents)
            self.assertIn("Do not block on `monitor-claude.sh`", agents)
            self.assertIn("hard timeout expires an execution epoch", agents)
            self.assertIn("Every changed byte", agents)
            self.assertIn("Tools establish facts; models make claims", agents)
            self.assertIn("workflow bypassed:", agents)
            self.assertIn("Checker/Test Claude", agents)
            self.assertIn("Unknown visibility fails closed", agents)
    ...
    def test_installed_dispatch_has_observability_and_timeout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('AI_WORKFLOW_TASK_MODE', dispatch)
            self.assertIn('aiwf-execution-card-v1', dispatch)
            self.assertIn('task-mode=', dispatch)
            self.assertIn('builder-mode=', dispatch)
    ...
    def test_installed_checker_helper_and_review_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("--prompt-mode capsule", review)
            self.assertIn("--spark-compression", review)
            self.assertIn("AI_WORKFLOW_SPARK_COMPRESSION", review)
            self.assertIn("build-spark-summary-capsule.py", review)
            ...
            self.assertIn("# Tool-backed review fallback", review)
            self.assertNotIn('DIFF_CONTENT="$(head -c', review)
            packet_builder = (repo / "ai" / "build-review-packet.py").read_text(encoding="utf-8")
            self.assertIn("# Tool-backed final review", packet_builder)
            self.assertIn("verify-evidence-capsule.py", packet_builder)
            self.assertIn("Do not cat the full packet", packet_builder)
            self.assertTrue((repo / "ai" / "evidence_capsule.py").is_file())
            self.assertTrue((repo / "ai" / "build-spark-summary-capsule.py").is_file())
            self.assertTrue((repo / "ai" / "verify-evidence-capsule.py").is_file())
            ...
            self.assertIn("component composer is for explicit legacy Markdown cards only", agents)
            self.assertIn("Codex episode ends", agents)
            self.assertIn("semantic_blocked", agents)
            self.assertIn("not Codex synchronization points", agents)
    ...
    def test_installed_claude_operations_helpers(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('--escalation-confirmations', watch)
            self.assertIn("authoritative Claude process identity", kill)
            self.assertIn("terminate-process", kill)
            self.assertIn("manual-stop.json", kill)
            self.assertNotIn('kill -9 "$PID"', kill)
    ...
    def test_installed_templates_include_token_budget_and_delegation_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # AGENTS.md keeps only the Bookend ownership contract; details are on-demand.
            self.assertIn("## Overnight Bookend Path", agents)
            self.assertIn("Claude owns exploration, implementation, assigned tests", agents)
            self.assertIn("diff coverage must be machine generated", agents)
