...
class InstallForCodexTests(unittest.TestCase):
    ...
    def test_skill_entrypoint_keeps_routing_context(self):
        ...
        self.assertIn("execution-modes.md", content)
    ...
    def test_installed_skill_keeps_progressive_disclosure_references(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((destination / "references" / "execution-modes.md").is_file())

    def test_three_mode_contract_preserves_intervention_boundaries(self):
        skill = (ROOT / "SKILL.md").read_text(encoding="utf-8")
        modes = (ROOT / "references" / "execution-modes.md").read_text(
            encoding="utf-8"
        )
        operating = (ROOT / "references" / "operating-model.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("Accepted `review_ready` is", skill)
        self.assertIn("terminal and does not schedule another Codex review", skill)
        self.assertIn("review after every returned round", skill)
        self.assertIn("Codex-native subagents", skill)
        self.assertIn("progress-based extensions", modes)
        self.assertIn("fixed `--window-minutes`", modes)
        self.assertIn("do not define", modes)
        self.assertIn("--reviewed-continuation APPROVAL", modes)
        self.assertIn("Interactive invokes neither Claude nor Spark", modes)
        self.assertIn("| `revision_pending` | yes |", operating)
        self.assertIn("| `review_ready` | no |", operating)
    ...
    def test_interactive_agent_specializations_are_optional(self):
        args = self.module.parse_args([])
        self.assertFalse(args.install_agents)
        source = (ROOT / "scripts" / "install_for_codex.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("Interactive mode works with built-in roles", source)
        self.assertIn("optional aiwf_* specializations", source)
