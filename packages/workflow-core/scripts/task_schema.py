...
VALID_PARTICIPANT_KEYS = {"id", "owner", "responsibilities"}
VALID_INTERFACE_KEYS = {
...
def _validate_known_extensions(
    ...
    validation_ids: Optional[set[str]] = None,
) -> None:
    ...
    if task_shape is not None:
        if not isinstance(task_shape, dict):
            ...
            participants = task_shape.get("participants")
            if participants is not None:
                participant_path = f"{shape_path}.participants"
                if not isinstance(participants, list):
                    errors.append(f"{participant_path}: expected array")
                elif not participants or len(participants) > 64:
                    errors.append(f"{participant_path}: expected 1 to 64 participant(s)")
                else:
                    for index, participant in enumerate(participants):
                        item_path = f"{participant_path}[{index}]"
                        if not isinstance(participant, dict):
                            errors.append(f"{item_path}: expected object")
                            continue
                        for key in participant:
                            if key not in VALID_PARTICIPANT_KEYS:
                                errors.append(f"{item_path}: unknown key '{key}'")
                        for key in ("id", "owner"):
                            value = participant.get(key)
                            if not isinstance(value, str) or not value.strip():
                                errors.append(f"{item_path}.{key}: expected non-empty string")
                        participant_id = participant.get("id")
                        if isinstance(participant_id, str) and participant_id.strip():
                            if participant_id in participant_ids:
                                errors.append(
                                    f"{item_path}.id: duplicate participant id '{participant_id}'"
                                )
                            participant_ids.add(participant_id)
                        _validate_text_array(
                            errors,
                            participant.get("responsibilities"),
                            f"{item_path}.responsibilities",
                            minimum=1,
                        )
            interfaces = task_shape.get("interfaces")
            if interfaces is not None:
                interface_path = f"{shape_path}.interfaces"
                if not isinstance(interfaces, list):
                    errors.append(f"{interface_path}: expected array")
                elif len(interfaces) > 256:
                    errors.append(f"{interface_path}: expected at most 256 interface(s)")
                else:
                    if not participant_ids:
                        errors.append(
                            f"{shape_path}.participants: required when interfaces are declared"
                        )
                    seen_interface_ids: set[str] = set()
                    for index, interface in enumerate(interfaces):
                        item_path = f"{interface_path}[{index}]"
                        if not isinstance(interface, dict):
                            errors.append(f"{item_path}: expected object")
                            continue
                        for key in interface:
                            if key not in VALID_INTERFACE_KEYS:
                                errors.append(f"{item_path}: unknown key '{key}'")
                        for key in ("id", "producer", "consumer", "owner", "contract"):
                        ...
                        if isinstance(interface_id, str) and interface_id.strip():
                            if interface_id in seen_interface_ids:
                                errors.append(
                                    f"{item_path}.id: duplicate interface id '{interface_id}'"
                                )
                            seen_interface_ids.add(interface_id)
                        producer = interface.get("producer")
                        consumer = interface.get("consumer")
                        if producer and producer == consumer:
                            errors.append(
                                f"{item_path}.consumer: producer and consumer must differ"
                            )
                        for key in ("producer", "consumer", "owner"):
                            reference = interface.get(key)
                            if isinstance(reference, str) and reference and reference not in participant_ids:
                                errors.append(
                                    f"{item_path}.{key}: references unknown participant '{reference}'"
                                )
                        validation_id = interface.get("validation_id")
                        if validation_id is not None:
                            if not isinstance(validation_id, str) or not validation_id.strip():
                                errors.append(
                                    f"{item_path}.validation_id: expected non-empty string"
                                )
                            elif validation_ids is not None and validation_id not in validation_ids:
                                errors.append(
                                    f"{item_path}.validation_id: references unknown validation id '{validation_id}'"
                                )
...
def _requires_complex_gate_contract(data: Dict[str, Any]) -> bool:
    ...
    if isinstance(shape, dict):
        ...
        participants = shape.get("participants", [])
        if isinstance(participants, list):
            for participant in participants:
                if isinstance(participant, dict):
                    values.extend(_text_items(participant.get("responsibilities")))
...
def assess_task_granularity(
    ...
    participants = [
        value for value in shape.get("participants", [])
        if isinstance(value, dict)
    ]
    participant_responsibilities = [
        "{}: {}".format(
            str(participant.get("id", "participant")).strip() or "participant",
            "; ".join(_text_items(participant.get("responsibilities"))),
        )
        for participant in participants
    ]
    ...
    responsibility_count = max(len(responsibilities), len(participants), 1)
    ...
    return {
        ...
        "responsibilities": responsibilities or participant_responsibilities or [task.get("goal", "task responsibility")],
...
def validate_task(data: Any, path: str = "") -> List[str]:
    ...
    if "extensions" in data and not isinstance(data["extensions"], dict):
        ...
        _validate_known_extensions(
            errors, data["extensions"], root, seen_validation_ids
        )
    extensions = data.get("extensions")
...
def _render_coordination_interfaces(task: Dict[str, Any]) -> str:
    """Render declared participant ownership and decomposition seams."""
    extensions = task.get("extensions", {})
    extensions = extensions if isinstance(extensions, dict) else {}
    shape = extensions.get("task_shape", {})
    shape = shape if isinstance(shape, dict) else {}
    participants = shape.get("participants", [])
    interfaces = shape.get("interfaces", [])
    parts: List[str] = []
    if isinstance(participants, list) and participants:
        ...
        if rows:
            parts.append("**Participants:**\n{}".format(
                _render_table(["ID", "Owner", "Responsibilities"], rows)
            ))
    if isinstance(interfaces, list) and interfaces:
        ...
        for interface in interfaces:
            ...
            rows.append([
                interface.get("id", ""),
                interface.get("producer", ""),
                interface.get("consumer", ""),
                interface.get("owner", ""),
                interface.get("contract", ""),
                interface.get("validation_id", ""),
            ])
        if rows:
            parts.append("**Interfaces:**\n{}".format(_render_table(
...
def _render_execution_task_card(
    ...
    coordination_body = _render_coordination_interfaces(task)
    if coordination_body:
        sections.append(_render_section("Coordination Interfaces", coordination_body))
