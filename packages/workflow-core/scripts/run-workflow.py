...
class RunContext:
    ...
    def __init__(
        ...
        reviewed_continuation: Optional[Path] = None,
    ):
        ...
        self.reviewed_continuation = reviewed_continuation
        ...
        self.balanced_review: Optional[Dict[str, Any]] = None
    ...
    def write_result(
        ...
        result = {
            ...
            "balanced_review": _safe_path(self.run_dir / "balanced-review.json"),
...
def _spark_context_preflight(repo: Path, dispatch_card: Path) -> tuple[bool, str, str]:
    """Bind Spark to the exact source worktree before any model process starts."""
    if not dispatch_card.is_file():
        return False, "dispatch-card-missing", ""
    try:
        ...
        return False, "git-root-unavailable", ""
    if result.returncode != 0 or not result.stdout.strip():
        return False, "git-root-unavailable", ""
    declared = os.path.normcase(os.path.realpath(repo))
    observed_root = result.stdout.strip()
    observed = os.path.normcase(os.path.realpath(observed_root))
    if declared != observed:
        return False, "source-root-mismatch", observed_root
    return True, "verified", observed_root
...
def _run_spark_attempt(
    ...
    command = [
        ...
        "--context-worktree", str(repo.resolve()),
    ]
    try:
...
def phase_dispatch(ctx: RunContext) -> None:
    ...
    if spark_policy.get("invoke"):
        ...
        spark_context_valid, spark_context_reason, spark_context_root = (
            _spark_context_preflight(ctx.repo, dispatch_card)
        )
        spark_record = {
            "schema_version": 2,
            "attempted": False,
            ...
            "context_worktree": str(ctx.repo.resolve()),
            "source_root_verified": spark_context_valid,
            "source_root_verification_reason": spark_context_reason,
            "observed_source_root": spark_context_root or None,
        }
        ...
        if not circuit_open and not spark_context_valid:
            spark_record.update(
                continued_to_claude=True,
                skip_reason="skip.spark-context-invalid",
                final_state="auto_disabled",
                exit_code=64,
            )

        if (
            not circuit_open and spark_context_valid
            and cached_host and not ctx.spark_host_authority
        ):
            spark_record.update(
                ...
                host_handoff_action=(
                    "rerun-current-workflow-from-authorized-host-with---host-authority"
                ),
            )
        ...
        spark_exit = 64 if not spark_context_valid else 127
        spark_timed_out = False
        if not circuit_open and spark_context_valid and spark_helper.is_file():
        ...
        if handoff_needed:
            ...
            if not ctx.spark_host_authority:
                ...
                raise PhaseError(
                    ...
                    "once outside the sandbox with --host-authority",
                )
        ...
        if not circuit_open and spark_context_valid:
    ...
    preview = {
        ...
        "reviewed_continuation": (
            str(ctx.reviewed_continuation) if ctx.reviewed_continuation else None
        ),
    }
    ...
    if convergence_receipt and ctx.reviewed_continuation:
        raise PhaseError(
            "dispatch",
            "blocked",
            "Bookend and reviewed continuations are mutually exclusive",
        )
    if convergence_receipt and Path(convergence_receipt).is_file():
        cmd.extend(["--bookend-continuation", convergence_receipt])
    if ctx.reviewed_continuation:
        approval = ctx.reviewed_continuation.resolve()
        if not approval.is_file():
            raise PhaseError(
                "dispatch", "blocked", f"reviewed continuation not found: {approval}"
            )
        cmd.extend(["--reviewed-continuation", str(approval)])
    ...
    if ctx.profile == "balanced":
        # Bind the top-level Balanced mode to the established dispatcher
        # profile while preserving an explicit supported operator override.
        dispatch_env.setdefault("CLAUDE_CODE_EXECUTION_PROFILE", "balanced")
        dispatch_env["AIWF_TOP_LEVEL_MODE"] = "balanced"
...
def phase_balanced_review(ctx: RunContext) -> None:
    """Build the deterministic handoff for one tuned Balanced round."""
    ctx.phase_order.append("balanced-review")
    start = time.monotonic()

    metrics_path = ctx.run_dir / "claude-phase-metrics.json"
    ...
    evidence_path = ctx.run_dir / "evidence.json"
    acceptance_path = ctx.run_dir / "acceptance-result.json"
    effective_profile = phase_metrics.get(
        "execution_profile",
        os.environ.get("CLAUDE_CODE_EXECUTION_PROFILE", "balanced"),
    )
    review = {
        "schema_version": 1,
        "kind": "balanced-round-review",
        "workflow_mode": "balanced",
        "task_id": (ctx.task_data or {}).get("id", ""),
        "round_status": "review_pending",
        "codex_review_required": True,
        "merge_authorized": False,
        "execution_owner": "claude",
        "intervention_boundary": "tuned-dispatcher-round-returned",
        "dispatcher_execution_profile": effective_profile,
        "time_window_plan": {
            "source": "dispatcher-tuned-policy",
            "fixed_single_checkpoint": False,
            "execution_profile": effective_profile,
            "context_acquisition_timeout_seconds": phase_metrics.get(
                "context_acquisition_timeout_seconds"
            ),
            "active_execution_window_seconds": phase_metrics.get(
                "active_execution_window_seconds"
            ),
            "hard_timeout_seconds": phase_metrics.get("hard_timeout_seconds"),
            "first_progress_timeout_seconds": phase_metrics.get(
                "first_progress_timeout_seconds"
            ),
            "no_output_timeout_seconds": phase_metrics.get(
                "no_output_timeout_seconds"
            ),
            "active_progress_extension_seconds": phase_metrics.get(
                "active_progress_extension_seconds"
            ),
            "growing_progress_extension_seconds": phase_metrics.get(
                "growing_progress_extension_seconds"
            ),
            "growth_extension_policy": phase_metrics.get(
                "growth_extension_policy"
            ),
            "context_acquisition_seconds_observed": phase_metrics.get(
                "context_acquisition_seconds"
            ),
            "active_execution_seconds_observed": phase_metrics.get(
                "implementation_seconds"
            ),
            "tail_seconds_observed": phase_metrics.get("tail_seconds"),
            "completion_ready_observed": phase_metrics.get(
                "completion_ready_observed"
            ),
        },
        "evidence": {
            "path": str(evidence_path),
            "sha256": content_hash(evidence_path.read_bytes())
            if evidence_path.is_file()
            else None,
        },
        "acceptance": {
            "path": str(acceptance_path),
            "sha256": content_hash(acceptance_path.read_bytes())
            if acceptance_path.is_file()
            else None,
            "status": (ctx.acceptance or {}).get("status"),
        },
        "product_worktree": ctx.product_worktree,
        "round_kind": "revision" if ctx.reviewed_continuation else "initial",
        "reviewed_continuation": (
            {
                "path": str(ctx.reviewed_continuation.resolve()),
                "sha256": content_hash(ctx.reviewed_continuation.read_bytes()),
            }
            if ctx.reviewed_continuation
            and ctx.reviewed_continuation.is_file()
            else None
        ),
        "allowed_decisions": ["accept", "revise", "stop"],
        "next_round_requires_revision_delta": True,
        "next_round_protocol": [
            "freeze a bounded revision Task JSON and delta review packet",
            "prepare a one-use reviewed-continuation approval bound to the current worktree state",
            "run aiwf balanced NEXT_TASK.json --reviewed-continuation APPROVAL",
        ],
    }
    ctx.balanced_review = review
    out = ctx.run_dir / "balanced-review.json"
    write_artifact(out, review)
    ctx.record_artifact(out)
    ctx.phase_timings["balanced-review"] = time.monotonic() - start
    ctx.emit_event(
        "balanced_review_ready",
        "review",
        {
            "acceptance_status": review["acceptance"]["status"],
            "codex_review_required": True,
        },
    )
...
def run_lifecycle(
    ...
    In ``balanced`` profile, the dispatcher applies its tuned context,
    execution, progress-extension, and hard-deadline policy. Evidence and
    deterministic acceptance are collected when that round returns, then a
    ``balanced-review.json`` handoff schedules the active Codex thread. A task
    may use multiple bounded rounds.
    ...
    # Balanced runs one tuned dispatcher round and then builds deterministic
    # evidence for the active Codex reviewer. Standard retains the complete
    # foreground compatibility lifecycle.
    phase_funcs = [
        phase_lint,
        phase_compose,
        phase_validate,
    ...
    if profile == "balanced" and status == "completed":
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser.add_argument(
        "--host-authority",
        action="store_true",
        default=_truthy(os.environ.get("AI_WORKFLOW_HOST_AUTHORITY", "")),
        help=(
            "Assert this workflow already has explicit host authority for both "
            "the bounded Spark audit and Claude dispatch, avoiding a second handoff."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="standard",
        help=(
            "Execution profile. 'balanced' runs one Claude round under the "
            "dispatcher's tuned time-window plan, builds deterministic evidence, "
            "and returns balanced-review.json to the active Codex thread."
        ),
    )
    parser.add_argument(
        ...
        type=Path,
        help=(
            "One-use approval from `aiwf reviewed-continuation prepare`; "
            "Balanced revision rounds use it to reuse the reviewed worktree."
        ),
    )
    ...
    result = run_lifecycle(
        ...
        reviewed_continuation=args.reviewed_continuation,
    )
    ...
    if result.get("failure_status") == "needs-host-execution":
        return 75
    return 0 if result.get("status") in ("completed", "routed", "review_pending") else 1
