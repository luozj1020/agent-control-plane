...
function renderTaskContract(): string[] {
  return [
    "## Task Card",
    "",
    "Create a Workspace working copy from the JSON Task Card, then validate and freeze that exact generation before delegation:",
    "",
    "```bash",
    "agent-control-plane task init --output TASK.json",
    "agent-control-plane task create --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --task TASK.json --source MAIN_AGENT",
    "agent-control-plane task validate --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --expected-working-copy-generation GENERATION",
    "agent-control-plane task freeze --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --expected-working-copy-generation GENERATION",
    "```",
    "",
  ...
  switch (mode.kind) {
    case "overnight": {
      ...
      if (overnight.policy.strategy === "convergent") {
        return [
          ...shared,
          "1. Freeze the Workspace Task; do not delegate while acceptance, scope, or validation is ambiguous.",
          "2. Create an immutable Preflight Receipt from the active Task and Skill:",
          "",
          "```bash",
          `agent-control-plane task preflight --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --workflow-mode overnight --adapter ${overnight.builderId} --strategy convergent --wake-adapter durable-file`,
          "agent-control-plane overnight submit --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --preflight-id PREFLIGHT_ID",
          "```",
          "",
          ...
          "agent-control-plane overnight review --run RUN_DIR --decision revise --workspace ABSOLUTE_WORKTREE --delta REVISION-DELTA.json",
      ...
      return [
        ...shared,
        "1. Freeze the improvement purpose and initial Workspace Task. Its acceptance criteria are the permanent minimum floor; its forbidden paths and human-authority boundaries are immutable.",
        "2. Create the immutable Preflight Receipt, submit the first cycle, and retain `runDirectory`:",
        "",
        "```bash",
        `agent-control-plane task preflight --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --workflow-mode overnight --adapter ${overnight.builderId} --strategy continuous-improvement --wake-adapter durable-file`,
        "agent-control-plane overnight submit --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --preflight-id PREFLIGHT_ID",
        "```",
        "",
    ...
    case "balanced": {
      ...
      return [
        ...
        "## Procedure",
        "",
        "1. Freeze the Workspace Task, create an immutable Preflight Receipt, then run exactly one tuned foreground round:",
        "",
        "```bash",
        `agent-control-plane task preflight --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --workflow-mode balanced --adapter ${balanced.builderId}`,
        "agent-control-plane balanced run --workspace ABSOLUTE_WORKTREE --task-id TASK_ID --preflight-id PREFLIGHT_ID",
        "```",
        "",
        ...
        "agent-control-plane balanced review --run RUN_DIR --decision revise --workspace ABSOLUTE_WORKTREE --delta REVISION-DELTA.json",
        "```",
        "",
